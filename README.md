...
16) Autoencoder - It is a type of neural network that learns to copy the input to its output. Further it may look like an identity function but the ouptut x' is a constraint when compared to the input x. It learns efficient data encodings in an unsupervised manner. The input encoder gets the input and it pushed the input to lower dimensional space that is the latent vector (called as the bottleneck layer). It allows model to learn by performing dimensionality reduction, much better than PCA and allows model to learn powerful generalizations. Output is of less loss. While PCA is on a linear dataset, auto encoder is going to be performed on a non linear data. The decoder again decodes them to higher space. Differnece between the input vector and the ouput vector(reconstruction vector) is called as the reconstruction error. It usually differentiates between the normal and the abnormal data. Its sets a threshold and whether it is anomaly or not depends on the threshold. The anomaly detection can be a feed forward neural network and LSTM OR CNN
